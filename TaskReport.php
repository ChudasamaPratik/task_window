...
?>
<title>Task Report - .:: Task Window ::.</title>
    ...
    ?>
        ...
        <table class="table table-bordered mt-2" id="example">
            ...
            <tbody>
                ...
                $sql = "SELECT tasks.Id as tid, tasks.TaskTitle, tasks.DeptId, tasks.AssignTaskto, tasks.TaskEnddate, tasks.TaskAssigndate, departments.DepartmentName, departments.ID as did, employees.EmployeeName, employees.EmployeeUsername,projects.* FROM `tasks` JOIN `departments` ON departments.ID = tasks.DeptId JOIN `projects` ON projects.ID = tasks.ProjectId JOIN `employees` ON employees.Id = tasks.AssignTaskto WHERE tasks.TaskAssigndate BETWEEN '$from' AND '$to' AND tasks.DeptId = $dept";
                $data = $con->query($sql);
